...
cpu_info() {
    ...
    echo "$data" | while read -r line; do
        ...
        if awk "BEGIN {exit !($used > $cpu_threshold)}"; then
            echo "Abnormal CPU[$cpu_name] usage[$used%]"
        fi
    done
    ...
    if [ -n "$data" ]; then
        echo "$data" | while read -r line; do
            ...
            cmd=$(get_cmd $pid)
            echo "Abnormal process[$pid:$cmd] cpu usage[$used%]"
        done
    fi
}
...
mem_info() {
    ...
    if awk "BEGIN {exit !($swap_used > $mem_threshold)}"; then
        echo "Abnormal Swap usage[$swap_used%]"
    fi
    ...
    if [ -n "$data" ]; then
        echo "$data" | while read -r line; do
            ...
            cmd=$(get_cmd $pid)
            echo "Abnormal process[$pid:$cmd] memory usage[$used%]"
        done
    fi
}
        ...
        for pid in /proc/[0-9]*; do
            ...
            if [ -d "$pid" ]; then
                ...
                if [ $cnt -gt $fd_threshold ]; then
                    echo "Abnormal process[$prog_name] fd count[$cnt]"
                fi
            fi
        done
    else
        echo "must be root user to run [fd_info]"
    fi
}
